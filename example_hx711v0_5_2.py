...
def checkValue(longValue,weightValue):
    if longValue == -1:
        print("[WARN] invalid value")
    if weightValue < 0:
        print("[WARN] negative weight")
    if len(weightValues)>10 and weightValue > 1:
        last_10_avg = sum(weightValues[-10:])/10
        #print(last_10_avg)
        #print(weightValue)
        if weightValue < 0.75*last_10_avg:
            print("[INFO] sudden large weight shift down")
        elif weightValue > 1.25*last_10_avg:
            print("[INFO] sudden large weight shift up")
...
def getRawBytesAndPrintAll():
    ...
    longValue = hx.rawBytesToLong(rawBytes)
    longWithOffsetValue = hx.rawBytesToLongWithOffset(rawBytes)
    weightValue = round(hx.rawBytesToWeight(rawBytes))
    weightValues.append(weightValue)
    checkValue(longValue,weightValue)
